...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        ////////////////////////////////////////////////////////////////
        const string groupName = "hinges";
        const double setPoint = -35;
        ...
        const float tartStrength = 50000;
        const float strengthMult = 1000;
        ////////////////////////////////////////////////////////////////
        ...
        public void Main(string argument)
        {
            ...
            double driveCommand, currentValue, currentForce;
            for (int i = 0; i < blocks.Count; i++)
            {
                pids[i] = new PID(P, I, D, setPoint);
                currentValue = ToDeg((blocks[i] as IMyMotorStator).Angle);
                currentForce = (blocks[i] as IMyMotorStator).Torque;
                driveCommand = pids[i].getDriveCommand(currentValue);
                Echo(String.Format("{0} - {1:00.000} - {2:00.000} - {3:0.0}", i, currentValue, driveCommand, currentForce));
                (blocks[i] as IMyMotorStator).TargetVelocityRad = (float)ToRad(driveCommand);
                (blocks[i] as IMyMotorStator).Torque = tartStrength + (float)Math.Abs(driveCommand) * strengthMult;
            }
        }
